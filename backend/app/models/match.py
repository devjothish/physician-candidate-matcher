...
class MatchScore(BaseModel):
    ...
    explanation: str = Field(
        ..., description="Factual explanation for the score"
    )
...
class CandidateMatch(BaseModel):
    ...
    overall_score: float = Field(
        ..., ge=0.0, le=100.0, description="Weighted overall score 0-100"
    )
    rank: int = Field(default=0, ge=0, description="Rank among matched candidates")
    scores: list[MatchScore] = Field(
    ...
    gaps: list[str] = Field(
        default_factory=list, description="Gaps or concerns"
    )
